    ...
    if request.method == 'POST':
        ...
        if not check_password_hash(stored_hash, password):
            ...
            try:
                if fail_count == 2:
                    details = {
                        'reason': 'consecutive_failed_logins', 'username': username, 'ip': ip, 'failed_count': fail_count, 'window_minutes': LOCKOUT_WINDOW_MINUTES}
                    raise_anomaly(conn, user_id=row['id'], score=0.2, details=details, severity='low')
                    append_alert_to_file({"type": "auth_warning_low", "username": username, "ip": ip, "severity": "low", "details": details, "created_at": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")})
                    
                elif fail_count in (3, 4):
                    details = {'reason': 'consecutive_failed_logins', 'username': username, 'ip': ip, 'failed_count': fail_count, 'window_minutes': LOCKOUT_WINDOW_MINUTES }
                    raise_anomaly(conn, user_id=row['id'], score=0.6, details=details, severity='medium')
                    append_alert_to_file({"type": "auth_warning_medium","username": username,"ip": ip,"severity": "medium","details": details,"created_at": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")})
            except Exception as e:
                print("[warning] staged anomaly failed:", e)
                    
            if fail_count >= LOCKOUT_THRESHOLD:
                details = {'reason': 'failed_login_threshold','username': username,'ip': ip,'failed_count': fail_count,'window_minutes': LOCKOUT_WINDOW_MINUTES
                           }
                try:
                ...
                append_alert_to_file({"type": "auto_lockout","username": username,"ip": ip,"severity": "high","details": details,"created_at": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")})
...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            conn.close()
            flash(f"Account created as {role.title()}.", "success")
            return redirect(url_for('dashboard', username=username))
